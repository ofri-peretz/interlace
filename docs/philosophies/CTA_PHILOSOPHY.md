...
**Measure**: button-variant audit script — if any CTA in `apps/*/src` uses
...
> **Not yet enforced in this repo.** The two lock tests below live in the
> eslint monorepo (`apps/docs/src/__tests__/`), where this philosophy used to
> live. Nothing in interlace asserts the hero CTA contract today — porting
> them is open work.
>
> - `homepage-lock.test.tsx` — source-text asserts per JSX element (primary
>   has no `shimmer={false}`, secondary has both, plus the GitHub icon/label
>   and the `mb-16` gap).
> - `hero-section-render.test.tsx` — DOM render-time asserts via
>   `data-slot="shimmer-button"`, `data-shimmer-spark`,
>   `data-shimmer-highlight` seams on the actual mounted output.
