...
- **Use the DS primitive.** `<SkipLink>` from
  [`packages/ui/src/primitives/skip-link.tsx`](../../packages/ui/src/primitives/skip-link.tsx),
  rendered as the very first element in the document — first child of
  `<body>`, before any provider.
- **Visually hidden until focused.** Use `sr-only focus:not-sr-only` so
  ...
  pill the moment a Tab press lands on it. `<SkipLink>` already does.
- **Targets a stable id — `#main`.** That is the `href` default in
  `skip-link.tsx`, and what the app routes render
  (`apps/landing/src/app/not-found.tsx`, `apps/registry/src/app/error.tsx`,
  …). Every page exposes it on its `<main>` landmark — or, when a
  framework renders `<main>` for us, on a `<div id="main"
  tabIndex={-1}>` immediately inside. Pass `href` explicitly only when a
  consumer app has a different established id.
- **The target must be programmatically focusable.** `tabIndex={-1}` is
  required so `location.hash = "#main"` actually moves focus there;
  without it the URL hash updates but focus stays on the skip link.

**Measure**: grep — every app's root layout renders `<SkipLink>` and
every routable page has a `<main id="main" tabIndex={-1}>` (or
equivalent wrapper). Manual: with Safari Full Keyboard Access on, the
FIRST Tab press on `/` must reveal the skip pill in the top-left.
...
**Measure**: grep audit — `apps/*/src` and `packages/ui/src` should
contain ZERO `<div onClick>` or `<span onClick>` patterns. (Currently
passes.)
...
- **Use semantic ring colors.** `focus-visible:ring-fd-ring` (fumadocs
  theme token) on chrome; `focus-visible:ring-primary` on branded
...
**Measure**: grep — `apps/*/src` and `packages/ui/src` should contain
ZERO `tabIndex={N}` where `N > 0`. (Currently passes.)
...
1. **#1 Skip link** — does this page have a skip link to `#main`?
