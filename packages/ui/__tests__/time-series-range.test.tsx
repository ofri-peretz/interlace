...
describe('TimeSeries range comparison', () => {
  ...
  it('states the direction as data, not only as colour (WCAG 1.4.1)', () => {
    ...
    cursorTo(2);
    mark();
    expect(document.querySelector('[data-direction]')?.getAttribute('data-direction')).toBe('up');
  });
  ...
  it('reads a fall as a fall, whichever end was marked first', () => {
    ...
    cursorTo(0);
    mark();
    expect(readout().indexOf('2026-06-01')).toBeLessThan(readout().indexOf('2026-06-15'));
    expect(document.querySelector('[data-direction]')?.getAttribute('data-direction')).toBe('up');
  });
  ...
  it('honours polarity, because a rise is not good news on every chart', () => {
    // The default would paint a 100% rise green. On error rate, latency or
    // cost that is the opposite of the truth, and colour is the first thing
    // a reader takes from a chart.
    render(<TimeSeries points={RISING} label="error rate" unit="errors" polarity="inverse" />);
    cursorTo(0);
    mark();
    cursorTo(2);
    mark();
    const delta = document.querySelector('[data-tone]');
    expect(delta?.getAttribute('data-direction')).toBe('up');
    expect(delta?.getAttribute('data-tone')).toBe('bad');
  });

  it('defaults to normal polarity', () => {
    render(<TimeSeries points={RISING} label="npm downloads" unit="downloads" />);
    cursorTo(0);
    mark();
    cursorTo(2);
    mark();
    expect(document.querySelector('[data-tone]')?.getAttribute('data-tone')).toBe('good');
  });

  it('keeps announcing after the range is locked', () => {
    // The live region is a single `aria-live` output. If its content froze
    // once the comparison completed, every later arrow key would announce
    // nothing and a screen-reader user could not tell the keys still worked.
    render(<TimeSeries points={RISING} label="npm downloads" unit="downloads" />);
    cursorTo(0);
    mark();
    cursorTo(2);
    ...
    expect(readout()).not.toBe(locked);
    // …and the comparison itself is still there. Announcing by DROPPING the
    // range would be a different bug with the same green test.
    expect(readout()).toContain('→');
  });
});
