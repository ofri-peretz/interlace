 ...
 *
 * Marking a point (click, or Enter) sets an anchor; marking a second one turns
 * the readout into a delta — absolute change, percentage, direction — and
 * shades the span between them. A third mark starts over. It is the gesture
 * every stock chart has, and the reason to state that here is that most of
 * them implement it as a pointer DRAG, which no keyboard user can perform.
 * Both paths run through one `select()`, for the same reason both crosshair
 * paths run through `nearestSlot`: two selection routines eventually disagree
 * about what is selected, and the readout can only be right about one.
 *
 * The delta itself is rendered by `Delta`, not computed here. A chart that
 * derives its own sign, percentage and tone is a chart with a private opinion
 * about what "up" looks like, which is the thing a design system exists to
 * prevent — and `Delta` already carries the WCAG 1.4.1 answer, stating the
 * direction in its accessible name rather than only in colour.
 *
 ...
 * either end shows nothing at all — bridging it would report a change that
 * never happened.
 *
 ...
 */
export const SERIES_STYLE: readonly {
  stroke: string;
  fill: string;
  dash?: string;
}[] = [
...
const MARK: Record<
  AnnotationKind,
  { className: string; d: (x: number, y: number) => string }
> = {
  publish: {
...
export interface TimeSeriesProps extends Omit<
...
export const TimeSeries = React.forwardRef<HTMLElement, TimeSeriesProps>(
  function TimeSeries(
    {
      points,
      compare = NO_COMPARE,
    ...
    const px = React.useCallback(
      (slot: number) => PAD_LEFT + plot.x(slot),
      [plot],
    );
     ...
     *
     * One function for both input paths, for the same reason `nearestSlot`
     * is: two selection code paths eventually disagree about which point is
    ...
    const select = React.useCallback(
      (slot: number) => {
        ...
        // A complete range is replaced, not extended — the third click starts
    ...
    /** Slot under a pointer event, or null when the box has not been laid out. */
    const slotFrom = (
    ...
    // also carry `pointerType`, which is how the touch path below knows to
    // place the crosshair before selecting — a finger never hovered first.
    ...
    if (!line) {
      ...
      return (
        <figure
          ...
          data-min-viewport={String(MIN_VIEWPORT)}
          className={cn(
            "m-0 w-full rounded-lg border border-border p-6",
            className,
          )}
          {...props}
        >
          <p className="text-sm text-muted-foreground">
            {own === 0 ? "No data yet." : `Only ${own} point so far.`} A trend
            needs at least two observations, and history cannot be back-filled.
          </p>
        </figure>
      );
    }
      ...
      >
        ...
        <svg
          ...
          aria-label={`${drawn
            ...
            .join(
              " ",
            )} Focus this chart and use the left and right arrow keys to read individual values. Press Enter to mark a point, then Enter again on a second point to compare the two.`}
          tabIndex={0}
          onKeyDown={onKeyDown}
        ...
        >
          ...
          <g clipPath={`url(#${clipId})`}>
            {annotations.map((annotation) => {
              ...
              return (
                <g
                  key={`${annotation.t}-${annotation.label}`}
          ...
          {/* The comparison band. Drawn BEFORE the crosshair so the crosshair
            ...
            below already states the range in words — a shaded rectangle is
            ...
            <g data-slot="time-series-range" aria-hidden>
              ...
              {[anchor, rangeEnd].map((slot, index) => (
                <line
                  ...
                  strokeDasharray={
                    index === 1 && fixed === null ? "3 3" : undefined
                  }
                />
              ))}
            </g>
        ...
        >
          {xSlots.map((slot, index) => (
            <span
              key={plot.keys[slot]}
              className={
                keepAtNarrow(index, xSlots.length)
                  ? undefined
                  : "hidden sm:inline"
              }
            >
          ...
          <output aria-live="polite" className="font-medium text-foreground">
            {comparison ? (
              ...
              >
                ...
                <Delta points={comparison} unit={unit} percent />
              </span>
            ) : (
              readout
            )}
