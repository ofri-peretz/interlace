...
const mergeWithPrevious = (derived, previous) => {
  ...
  const merged = derived.map((entry) => {
    ...
    // `since` is STICKY. It means "the release this component first shipped
    // in", which is immutable the moment it is known — a component that
    // shipped in 1.0.0 cannot first appear in 1.1.0.
    //
    // It has to be pinned here because the derivation cannot compute it:
    // `firstReleaseContaining` walks `git tag --list '@interlace/ui@*'`, and
    // this package is distributed as copied source rather than published to
    // npm, so **those tags have never existed**. The lookup always returns
    // `[]` and every component falls through to the `packageVersion`
    // fallback, which was accidentally correct while that value was 1.0.0.
    // The first bump exposed it: all 140 pre-existing components had `since`
    // advanced 1.0.0 → 1.1.0 in one commit.
    //
...
const manifestPayload = (components, packageVersion) => ({
  $comment:
    'GENERATED by apps/registry/scripts/derive-component-versions.mjs from git history. Do not hand-edit `version` / `updated` — they will be overwritten. `since` is pinned on first record and never re-derived; `deprecated` IS hand-authored. Both are preserved across regeneration. See docs/philosophies/VERSIONING_PHILOSOPHY.md.',
  packageVersion,
  components,
});
