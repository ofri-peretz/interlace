...
{/* AUTO-GENERATED — run `npm run sync:philosophies` after editing the source.
    source: docs/philosophies/UX_PHILOSOPHY.md
    hash: eb29a9f3bdbf */}
...
- **Per-page OG images.** Generated at build time via the existing
  `/og` route. Title + plugin badge + brand chrome.
...
[`CTA_PHILOSOPHY.md`](?path=/docs/philosophy-cta--docs).

For keyboard accessibility — skip links, focus-visible, Tab order,
Safari Full Keyboard Access — see
[`KEYBOARD_PHILOSOPHY.md`](?path=/docs/philosophy-keyboard--docs).
