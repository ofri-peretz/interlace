...
{/* AUTO-GENERATED — run `npm run sync:philosophies` after editing the source.
    source: docs/philosophies/INTEROP_PHILOSOPHY.md
    hash: ce8af5468cbe */}

# Tooling Interop & Static-Analysis Philosophy

A standalone engineering philosophy. Where [DOCS_PHILOSOPHY.md](?path=/docs/philosophy-docs--docs)
governs what a rule looks like to a reader, and [URL_PHILOSOPHY.md](?path=/docs/philosophy-url--docs)
