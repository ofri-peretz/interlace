...
const meta: Meta<typeof TimeSeries> = {
  ...
  argTypes: {
    ...
    compare: {
      ...
      table: {
        type: { summary: "readonly ComparisonSeries[]" },
    ...
    annotations: {
      ...
      table: {
        type: { summary: "readonly Annotation[]" },
        defaultValue: { summary: "[]" },
        category: "Data",
      },
    },
    ...
    unit: {
      control: "text",
      description:
        'Noun appended to the crosshair readout — "2026-07-14 · 2,205 downloads".',
      table: { type: { summary: "string" }, category: "Data" },
    },
    height: {
...
export const KeyboardCrosshair: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step("ArrowRight reads out a specific value", async () => {
      ...
      await waitFor(() =>
        expect(canvasElement.querySelector("output")?.textContent).toMatch(
          /\d/,
        ),
      );
    });
    ...
    await step("Escape clears the crosshair (WCAG 2.1.2)", async () => {
      ...
      await waitFor(() =>
        expect(canvasElement.querySelector("output")?.textContent).toBe(""),
      );
    });
  },
};
...
export const RangeComparison: Story = {
  parameters: {
    docs: {
      description: {
        story:
          "Mark two points and the readout becomes a comparison — absolute change, percentage, and a tone token that says which way it went. The same gesture on both input paths: click twice with a pointer, or press Enter twice with the keyboard. A third selection starts over.\n\n" +
          'The delta is rendered by `Delta`, not re-derived here, so this chart cannot develop its own opinion about what "up" looks like.',
      },
    },
  },
  args: { points: RISING, label: "npm downloads", unit: "downloads" },
  play: async ({ canvasElement, step }) => {
    ...
    await step(
      "the direction is exposed as data, not only as colour",
      async () => {
    ...
    await step("a third selection starts a new range", async () => {
      await userEvent.keyboard("{Home}");
      await userEvent.keyboard("{Enter}");
      await waitFor(() => expect(output()?.textContent).not.toMatch(/→/));
    });
    ...
    await step(
      "Escape clears the selection as well as the crosshair",
      async () => {
        await userEvent.keyboard("{End}");
        await userEvent.keyboard("{Enter}");
        await waitFor(() => expect(output()?.textContent).toMatch(/→/));
        await userEvent.keyboard("{Escape}");
        await waitFor(() => expect(output()?.textContent).toBe(""));
      },
    );
  },
};
...
export const TwoSeriesCrosshair: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step(
      "End reaches a slot where both series have a value",
      async () => {
        await userEvent.keyboard("{End}");
        await waitFor(() =>
          expect(canvasElement.querySelector("output")?.textContent).toContain(
...
export const NotEnoughData: Story = {
  parameters: {
    docs: {
      description: {
        story:
          "Says why it cannot plot. History cannot be back-filled, and an empty box reads as a bug.",
      },
    },
  },
...
export const MinViewportAxis: Story = {
    ...
    (Story) => (
      ...
      <div
        style={{
          width: 320,
          outline:
            "1px dashed color-mix(in oklch, currentColor 25%, transparent)",
        }}
      >
        <Story />
      </div>
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step(
      "the labels are legible, not viewBox-scaled to 4px",
      async () => {
        await expect(
    ...
    await step("the first label starts where the plot does", async () => {
      ...
      await expect(
        Math.abs(visible[0].getBoundingClientRect().left - expected),
      ).toBeLessThan(2);
    });
